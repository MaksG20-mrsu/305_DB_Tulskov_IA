...
with open('Task02/users.txt', 'r', encoding='utf-8') as f:
    for line in f:
        line = line.strip()
        if line:
            parts = line.split('|')
...
with open('Task02/tags.csv', 'r', encoding='utf-8') as f:
...
with open('Task02/movies.csv', 'r', encoding='utf-8') as f:
    reader = csv.DictReader(f)
    for row in reader:
        movie_id = row['movieId']
...
with open('Task02/ratings.csv', 'r', encoding='utf-8') as f:
    reader = csv.DictReader(f)
    for row in reader:
        user_id = row['userId']
        movie_id = row['movieId']
...
sql_script = '\n'.join(sql_script)
with open('Task02/db_init.sql', 'w', encoding='utf-8') as f:
    f.write(sql_script)